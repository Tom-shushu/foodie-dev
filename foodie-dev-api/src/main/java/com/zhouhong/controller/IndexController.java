...
import com.zhouhong.enums.YesOrNo;
...
public class IndexController {
     ...
     * 2.如果鼠标上移到大分类，则加载其子分类的内容，如果存在子分类则不需要加载
     */
    @ApiOperation(value = "获取首页商品分类（一级分类）",notes = "获取首页商品分类（一级分类）",httpMethod = "GET")
    @GetMapping("/cats")
    public JSONResult cats(){
        List<Category> list = categoryService.querAllRootLeveCat();
        return JSONResult.ok(list);
    }
    ...
    @ApiOperation(value = "查询每个一级分类下的六条最新商品",notes = "查询每个一级分类下的六条最新商品",httpMethod = "GET")
